...
// ---- IPC ----
api.onInit(({ collapsed, mode, locale, theme }) => {
...
api.onTokens((tk) => {
  if (!tk) return;
  const t = tk.totals || {};
  $('tok-in').textContent = fmtTok(t.input);
  $('tok-out').textContent = fmtTok(t.output);
  $('tok-cr').textContent = fmtTok(t.cacheRead);
  $('tok-cw').textContent = fmtTok(t.cacheWrite);
  $('tok-cost').textContent = '$' + (tk.cost || 0).toFixed(2);
  // per-model split: % of today's usage + cost. opus and sonnet always; haiku if used.
  const bm = tk.byModel || {};
  const total = Object.values(bm).reduce((s, v) => s + (v.cost || 0), 0);
  const rows = [];
  for (const m of ['opus', 'sonnet', 'haiku']) {
    if (m === 'haiku' && !bm[m]) continue;
    const cost = bm[m] ? bm[m].cost : 0;
    const pct = total > 0 ? Math.round((cost / total) * 100) : 0;
    rows.push(`<div class="bm-row"><span class="bk">${m}</span><span class="bv"><span class="bpct">${pct}%</span>$${cost.toFixed(2)}</span></div>`);
  }
